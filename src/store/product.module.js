...
export const productManage = {
   ...
   */
    ...
    */
    deleteProductByID({commit},product_id) {
    ...
    */
    updateProductByID({commit}, product) {
     ...
     * ---------save ThmubNail------------
     */
     saveThumbNail({commit},[product_id, thumbnail]){
     ...
     * ---------remove ThmubNail------------
     */
     removeThumbNail({commit}, product_id){
     ...
     */
    getCategoriesForProductID({commit}, product_id) {
     ...
     */
    getProductProgressID({commit}, product_id) {
      return ProductService.getProductProgressID(product_id).then(
        res => {
          if(res.status ===200){
            commit('getProductProgressIDSuccess', {res, product_id});
          }
          else{
            if(res.response == undefined)
                commit('NETWORK_ERROR', null, { root: true });
     ...
     * ---------- get product progress details
     */
    getStudentProgress({commit}, [product_id, student_id]) {
  ...
  mutations: {
    ...
    getProductProgressIDSuccess(state, {res, product_id}) {
      store.state.status = { got: true};
      store.state.notification_icon = "info"
      store.state.notification_color = "primary"
