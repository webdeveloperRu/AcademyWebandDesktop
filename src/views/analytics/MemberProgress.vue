    ...
    <vs-row vs-justify="center">
      <vs-col vs-sm="12" vs-lg="12">
        <span
          ...
          ><i class="ti-angle-left" style="font-size: 14px;"></i> All
          Members</span
        >
        <h2 class="mt-3" style="color: #333; font-weight: 600">
          Memeber Progress: producttitle
        </h2>
        <br />
      </vs-col>
    </vs-row>
        ...
        <vs-card actionable class="cardx">
          <div class="d-flex" style="align-items: center">
            ...
            <div class="ml-3">
              ...
              <div>
                <a :href="mail_current_student">{{
                  current_student_progress.student.email
                }}</a>
              </div>
              <div><strong>Progress</strong>: {{ total_progress }} %</div>
              <div>
                <strong>Last Login</strong>:
                {{ current_student_progress.student.last_activity }}
              </div>
            </div>
          </div>
        </vs-card>
            ...
            >
              ...
              <template slot-scope="{ data }">
                <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
                  ...
                  <vs-td>{{ data[indextr].completed }} %</vs-td>
                </vs-tr>
              </template>
            </vs-table>
          </div>
      ...
      >
        <vs-card actionable class="cardx">
          <div class="d-flex" style="justify-content: space-between">
            ...
            <div>
              Completed {{ completed_lessons }} of
              {{ selected_category.lessons.length }} Lessons
            </div>
          </div>
          ...
          <vs-row
            ...
            class="mt-3 mb-3"
            v-bind:key="index"
            v-for="(data, index) in selected_category.lessons"
          >
            ...
            <vs-col vs-lg="7">
              ...
              <vs-progress :percent="data.completed" color="primary"
                >primary</vs-progress
              >
            </vs-col>
...
export default {
  ...
  computed: {
    ...
    total_progress: {
      get() {
        ...
        for (
          let i = 0;
          i < this.current_student_progress.progress.length;
          i++
        ) {
  ...
  methods: {
    ...
    getStudentProgress(product_id, student_id) {
      this.$store
        .dispatch("productManage/getStudentProgress", [product_id, student_id])
        .then(() => {
