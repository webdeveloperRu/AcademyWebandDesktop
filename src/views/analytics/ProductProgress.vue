...
<div>
 <vs-row vs-justify="center" class="progressbar">
    <vs-col vs-sm="12" vs-lg="11">
      <span  style="cursor: pointer;" class="ml-2 mb-5 mt-2 primary-font" @click="linkToProducts"><i class="ti-angle-left" style="font-size: 14px;"></i> products</span>
      <h2 class="mt-3" style="color: #333; font-weight: 600">Product Progress: {{current_product_title}}</h2>
      <br />
  ...
  <vs-row vs-justify="center" class="responsive_progress" >
    <vs-col vs-lg="8" vs-xs="12" vs-sm="8" id="printbody">
      <vs-card>        
        <vs-table :data="progressData" class="text-nowrap" :key="updatedProducts" pagination maxItems="20">
          <template slot="thead">
          ...
          <template slot-scope="{data}">
            <vs-tr class="border-top" :data="tr" :key="indextr" v-for="(tr, indextr) in data">
              ...
              <vs-td>
                <div @click="linkToMemberProgress(product_id)" class="member-name">{{ data[indextr].memberName }}</div>
              </vs-td>
              <vs-td>
                <vs-progress :percent="data[indextr].progressValue" color="primary">primary</vs-progress>
              </vs-td>
    ...
    <vs-col vs-lg="3" vs-xs="12" vs-sm="4" class="filter-sticky">
      <vs-card class="">
        ...
        <vs-select class="w-100 mt-3" label="Product" v-model="product" :key="updatedProducts">
          <vs-select-item
            :key="index"
            :value="item.value"
            :text="item.text"
        ...
        <vs-input label="Memeber Name" v-model="member_name" class="w-100 mt-3"></vs-input>
        <vs-input label="Memeber Email" v-model="member_email" class="w-100 mt-3"></vs-input>
        <div class="btn-alignment text-right mt-3">
          <vs-button color="primary" type="flat" @click="searchProductProgress" >Apply</vs-button>
        </div>
      </vs-card>
    </vs-col>
...
export default {
  ...
  methods: {
    ...
    linkToMemberProgress(student_id) {
      this.$router.push('/analytics/member-progress/' + this.product_id +'/' + student_id)
    },
    ...
    getProudctProgress() {
      this.$store.dispatch('productManage/getProductList').then(async ()=>{
        ...
        for ( let i = 0; i < this.product_list.length; i++) {
          this.products[i] = {text: this.product_list[i].title, value: this.product_list[i].id}
          // await this.$store.dispatch('productManage/getProductProgressID', this.product_list[i].id)
        }
        await this.$store.dispatch('productManage/getProductProgressID', this.product_id)
        ...
        for ( let j = 0; j < progress_data.length; j++) {
          this.progressData[j] = {
            ...
            progressValue: progress_data[j].progress.done*100/progress_data[j].progress.total,
          }
        }
        this.updatedProducts++;
    ...
    async searchProductProgress() {
      if (this.product_progress[this.product] == undefined) {
        this.$vs.loading({type: 'material'});
        await this.$store.dispatch('productManage/getProductProgressID', this.product)
        this.$vs.loading.close()
      }
      ...
      for ( let j = 0; j < progress_data.length; j++) {
        ...
        } else if (progress_data[j].full_name.toUpperCase().includes(this.member_name.toUpperCase())) {
          if(this.member_email == '') {
            this.progressData[count] = {
              memberName: progress_data[j].full_name,
              loginCount: progress_data[j].log_in_count,
