<template>
<vs-row class="" style="display: flex; margin-top: 12%;
  justify-content: center;
  align-items: center;">
  <vs-card style="width: 700px;" class="mx-auto">    
    <h1 class="ml-3 mt-3">Two-Factor Authentication</h1>
    <br>
    <div class="mt-2 mb-5 ml-3" style="font-size: 20px;">Please enter the 2fa code from your mobile device below.</div>
    <br><br>
    <div class="pf-input-pin__wrapper mt-3">
      <input class="two-factor-large-input" maxlength="1" id="login2faInputPin-0" tabindex="-1" autocomplete="off"  v-model="pincode1" ref="input_pincode1" @keydown="processDeleteKey($event)" @click="processFocus">
      <input class="two-factor-large-input ml-4" maxlength="1" id="login2faInputPin-1" tabindex="-1" autocomplete="off"  v-model="pincode2" ref="input_pincode2" @keydown="processDeleteKey($event)" @click="processFocus">
      <input class="two-factor-large-input ml-4" maxlength="1" id="login2faInputPin-2" tabindex="-1" autocomplete="off"  v-model="pincode3" ref="input_pincode3" @keydown="processDeleteKey($event)" @click="processFocus">
      <input class="two-factor-large-input ml-4" maxlength="1" id="login2faInputPin-3" tabindex="-1" autocomplete="off"  v-model="pincode4" ref="input_pincode4" @keydown="processDeleteKey($event)" @click="processFocus">
      <input class="two-factor-large-input ml-4" maxlength="1" id="login2faInputPin-4" tabindex="-1" autocomplete="off"  v-model="pincode5" ref="input_pincode5" @keydown="processDeleteKey($event)" @click="processFocus">
      <input class="two-factor-large-input ml-4" maxlength="1" id="login2faInputPin-5" tabindex="0" autocomplete="off"   v-model="pincode6" ref="input_pincode6" @keydown="processDeleteKey($event)" @click="processFocus">
      
    </div>
    <br>
    <button class="mt-5 two-factor-button mb-4 mr-3" v-on:click="login">Log in</button>
  </vs-card>
</vs-row>
</template>
<script>
export default {
  ...
  methods: {
    login: function(){
      ...
      this.$store.dispatch('auth/login', [this.login_userinfo, this.rememberme] ).then(()=>{
        if(this.user_logged){
            if(!this.is_disabled){
              ...
              this.$vs.notify({
                color: '#ff3300',
                text: 'this is user disabled now',
                icon: 'warning'
              })
            }
        } else{
