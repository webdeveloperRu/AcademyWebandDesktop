  ...
  <vs-row vs-justify="center">
    ...
    <vs-row>
      <div class="ml-3 mb-4">
        <div class="d-flex">
          <h2>Offers</h2>
          <i class="mdi mdi-help-circle-outline ml-1"
            style="color: #3b66ff; font-size: 22px"
          ></i>
        </div>
        <div class="d-flex" >
          <h5 class="ml-2 mt-2" @click="linkToCouponPage" style="cursor: pointer; font-size: 14px"><i class="mdi mdi-ticket"></i> Coupons</h5>
          <h5 class="ml-2 mt-2" @click="linkToPayment" style="cursor: pointer; font-size: 14px"><i class="mdi mdi-currency-usd"></i> Payments</h5>
        </div>
      </div>
      <vs-button
            ...
            <template slot-scope="{ data }">
              ...
              >
                <vs-td class="offer-th">
                  <div class="offer-thumbnail-field"  id="offerviewfield" :style="{ 'background-image': getImageUrl(data[indextr].thumbnail) }"> 
                ...
                <vs-td class="offer-th" :data="data[indextr].price">
                  {{ data[indextr].price }} ({{data[indextr].currency | capitalize}})
                </vs-td>
                ...
                <vs-td>
                  <div class="offer-action-icon">
                    <i 
                      ...
                      :style="data[indextr].status=='published'? 'color: #08800c;': ''"
                      @mouseover="onCheckIconMouseOver"
                      @click="onCheckIconClick"
                    ></i>
                    <a :href="getUrl(data[indextr].id)" target="_blank" class="preview-checkout"> 
                      <i class="mdi mdi-eye mr-4 product-processing-icon"
                        style="font-size: 16px"
                     ...
                     <vs-dropdown-menu>
                        ...
                        <vs-dropdown-item>
                          <div class="ml-3 mr-3 primary-font" 
                            style="font-size: 16px" @click="editCheckout(data[indextr].id)">
                            Checkout
                          </div>
                        </vs-dropdown-item>
                      </vs-dropdown-menu>
    ...
    <!-- 
      ***@  --------add offer ---------------
     -->
    <vs-popup color="success" :active.sync="activeAddOffer" title="New Offer" class="offer-new-pop">
      <vs-row class="px-5">
        <vs-col>
          ...
          <Multiselect v-model="selectedOfferProduct" tag-placeholder="Add this as new tag" placeholder="Search or add a product" label="name" track-by="code" :options="products" :multiple="true" :taggable="true"></Multiselect>
          <span>Which product would you like to sell with this offer</span>
          <h5 class="mt-3 mb-2"> Price </h5>
          <vs-row>
            <vs-col vs-lg="8" class="mr-0 pr-0">
              <vs-input v-model="offerPrice" type="number" class="new-offer-price w-100" ></vs-input>
            </vs-col>
  ...
  methods: {
    ...
    initProducts () {
      for (let i = 0; i < this.product_list.length; i++) {
        this.products[i] = {name: this.product_list[i].title, code: this.product_list[i].id}
      }
      this.getOfferList();
     },
     /*
     ...
     */
     ...
     getOfferList() {
      this.$store.dispatch("offerManage/getOfferList").then(
        () => {
    ...
    addOffer() {
      ...
      this.offer.currency = this.currencyOptions[this.selected_currency].currency
      if(this.offerTitle == ''){
        this.$vs.notify({
      ...
      this.$store.dispatch("offerManage/addOffer", this.offer).then(
        () => {
          this.$vs.notify({
            color: this.notification_color,
            text: this.notification_text,
    ...
    getPurchaseAndNetrevenue() {
      for (let i = 0; i < this.offer_list.length; i++ ) {
        this.$store.dispatch("offerManage/getOfferStats", [this.offer_list[i].id, 30]).then(() => {
          this.net_revenue30 += this.offer_stats[this.offer_list[i].id].netRevenueCustom  
          this.net_revenueall += this.offer_stats[this.offer_list[i].id].netRevenueAllTime
          this.offer_purchases += this.offer_stats[this.offer_list[i].id].total_purchases 
        })
