  ...
  <div>
    <vs-row vs-justify="center" class="primary-font">
      <vs-col
        ...
        class="mb-3"
      >
        <span  @click="backToCurrentProduct" style="cursor: pointer;" class="ml-2 mb-5 mt-2 primary-font"><i class="ti-angle-left" style="font-size: 14px;"></i> {{ product_title }}</span>
      ...
      -->
        ...
        <vs-card class="category-edit">
          <ProductEditSubCategory  :id="category_id" title="Contents" :category_id="category_id" :category="current_category" :key="loaded"></ProductEditSubCategory>
        </vs-card>
          ...
          <div class="d-flex">
            <div class="">
              <vs-radio
                ...
                >Published</vs-radio
              >
              <vs-radio v-model="category_status" vs-name="category_status" vs-value="draft"
                >Draft</vs-radio
              >
              <vs-radio
                v-model="category_status"
              ...
              <div class="text-left mb-3 description-font ml-3">
                This category will become locked until the selected post is completed.
              </div>
              <vs-radio
                v-model="category_status"
                vs-name="category_status"
              ...
              >
              <div class="text-left mb-3 description-font ml-3">
                This category will become available for your memebers on a specified number of days after they acquire the product.
              </div>
              <div class="ml-5 mr-5" style="border: 1px solid #bbbbbb; padding: 20px; border-radius: 5px">
                <vs-input label="Number of Days" v-model="drip_days" type="number" class="w-100"></vs-input>
                <span class="description-font">This value should be greater than or equal 1. It should be less than or equal to 731.</span>
              </div>
            </div>
          </div>
        </vs-card>
    ...
    <vs-row vs-justify="center" class="primary-font">
      ...
      >
        <div class="btn-alignment text-right">
          <vs-button color="danger" type="flat" @click="deleteCurrentCategory"> Delete Category</vs-button>
          <vs-button color="primary" type="filled" @click = "saveCurrentCategory">Save</vs-button>
        </div>
      </vs-col>
    </vs-row>
  </div>
...
export default {
  ...
  methods: {
    updateCategory() {
      ...
      this.$store.dispatch('categoryManage/getCategoryByID', this.category_id).then(()=>{
        if(this.status_got) {
          this.current_category = this.$store.getters["categoryManage/current_category"]
          ...
          this.cssthumbImageUrl = "url(" + this.current_category.thumbnail +")";
    ...
    saveCurrentCategory() {
      ...
      this.$store.dispatch('categoryManage/updateCategoryByID', this.current_category).then(()=>{
          this.$vs.notify({
              color: this.notification_color,
              text: this.notification_text,
              icon: this.notification_icon
    ...
    backToCurrentProduct() {
      this.$router.push("/products/edit-product/" + this.current_category.product_id)
    },
    ...
    **/
    ...
    saveThumbNail (thumbFile) {
      this.$store.dispatch('categoryManage/saveThumbNail',[this.category_id, thumbFile]).then(()=>{
        this.$vs.notify({
          color: this.notification_color,
          text: this.notification_text,
          icon: this.notification_icon
    ...
    **/
    removeThumbNail () {
      this.$store.dispatch('categoryManage/removeThumbNail',this.category_id).then(()=>{
        this.$vs.notify({
          color: this.notification_color,
          text: this.notification_text,
          icon: this.notification_icon
