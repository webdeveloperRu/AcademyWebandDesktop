<template>
  <vs-row vs-justify="center" class="primary-font">
    ...
    >
      ...
      <h2 class="mt-3" style="color: #333; font-weight: 600">{{product_title}}</h2>
      <br />
      <div style="font-size: 14px; cursor: pointer; line-height:14px !important; " class="mb-3">
          <span class="primary-font"  @click="editProductDetails">
            <i class="mdi mdi-pencil mr-2 primary-font"></i> Edit details 
          </span>
        <a :href="current_product.preview_link"><span class="primary-font"><i class="mdi mdi-eye mr-2 ml-3 primary-font"></i> Preview </span></a>       
          ...
          <vs-dropdown-menu>
            ...
            <vs-dropdown-item>
              <div class="ml-3 mr-3 primary-font" 
                style="font-size: 14px; line-height:14px; font-weight:400" @click="manageComments">
                Manage Comments
              </div>
            </vs-dropdown-item>
             <!-- manage comment -->
            <vs-dropdown-item>
              <div class="ml-3 mr-3 primary-font" 
                style="font-size: 14px; line-height:14px; font-weight:400" @click="linkToProductProgress">
                Product Progress
              </div>
            </vs-dropdown-item>
            ...
            <vs-dropdown-item>
              <div class="ml-3 mr-3 primary-font" 
                style="font-size: 14px;  line-height:14px; font-weight:400"  @click="confirmDeleteProduct = true">
                Delete
              </div>
            </vs-dropdown-item>
      ...
      <vs-card actionable>
        <div class="d-flex" >
          ...
          <i class="mdi mdi-plus-circle-outline ml-2"
        ...
        <div>
          <vs-list id="product-category">
            ...
            <draggable :list="current_categories" class="cursor-move" group="category-group" @change="updateCategoryOrder">
              <transition-group >
                <vs-list-item
                ...
                >
                  <ProductEditSubCategory  :id="listitem.id" :title="listitem.name" :category_id="listitem.id" :category="listitem" :key="loaded"></ProductEditSubCategory>
                </vs-list-item>
              </transition-group>
            </draggable>
          </vs-list>
     ...
     <vs-popup
      ...
      <vs-row>
        <vs-col>
          ...
          <vs-input v-model="categoryTitle" class="w-100 mt-3 mb-5" ref="category_create" v-on:keyup.enter="addCategory"></vs-input>
          <div class="d-flex">
            <vs-button color="dark" type="border" class="save-cancel-button" style="margin-left:auto;" @click="activeAddCategory=false">Cancel</vs-button>
            <vs-button class="ml-3 mr-3 save-cancel-button" @click="addCategory">Save</vs-button>
          </div>
        </vs-col>
      </vs-row>
     </vs-popup>
     ...
     <!-- 
      ...
      <vs-popup class="product-delete" title="" :active.sync="confirmDeleteProduct">
        <div style="font-size: 25px; color: white" class="px-5 bg-danger m-0 ">
          <i class="mdi mdi-alert"></i> Are you sure you want to delete <strong>"{{current_product.title}}"</strong> ?
...
export default {
  ...
  methods:{
    ...
    updateCategoryList() {
      ...
      if( this.current_categories == undefined)
    ...
    updateCategoryOrder() {
      ...
      this.$store.dispatch('categoryManage/updateCategoryOrder',[updatedList, this.product_id]).then(()=>{
        this.$vs.notify({
    ...
    addCategory() {
      ...
      if(this.category.name ===''){
        ...
        this.$store.dispatch('categoryManage/addCategory', [this.category, this.product_id]).then(()=>{
          this.$vs.notify({
            color: this.notification_color,
            text: this.notification_text,
            icon: this.notification_icon
    ...
    getProductByID (product_id) {
      this.$store.dispatch('productManage/getProductByID', product_id).then(()=>{
        if(!this.status_got){
          this.$vs.notify({
            color: this.notification_color,
            text: this.notification_text,
    ...
    async getCategoriesForProductID(product_id) {
      ...
      await this.$store.dispatch('productManage/getCategoriesForProductID', product_id).then( async ()=>{
          if(!this.status_got){
            this.$vs.notify({
              color: this.notification_color,
              text: this.notification_text,
     ...
     **/
     getLessonList(category_id){
      return new Promise ((resolve, reject) => {
       this.$store.dispatch('lessonManage/getLessonList', category_id).then(()=>{
          if(!this.status_got){
            this.$vs.notify({
              color: this.notification_color,
              text: this.notification_text,
   ...
   */
  linkToProductProgress() {
    this.$router.push('/analytics/product-progress/' + this.current_product.id)
  }
