      ...
      <vs-col vs-lg="10" vs-sm="12">
        <vs-row>
          <div class="ml-2 mb-3 ">
            ...
            <div>
              <h5 class="ml-2 mt-2" @click="linkToComment" style="cursor: pointer; font-size: 14px"><i class="mdi mdi-comment"></i> Comments</h5>
            </div>
          </div>
          <vs-button style="margin-left: auto; height: 45px" class="mr-2" @click="addProductPopup">+ Add Product</vs-button>
        </vs-row>
        
        <vs-row vs-justify="left">
          <ProductPreview v-for="product in product_list" :key="product.id" :product="product"></ProductPreview>
        </vs-row>
       ...
       <vs-popup
        ...
        <vs-row>
          <vs-col>
            ...
            <vs-input v-model="productTitle" class="w-100 mt-3 mb-4" v-on:keyup.enter="addProduct" ref="product_create" ></vs-input>
            <div class="d-flex mb-4">
              <vs-button color="dark" type="border" class="save-cancel-button" style="margin-left:auto;" @click="activeAddProduct=false">Cancel</vs-button>
              <vs-button class="ml-3 save-cancel-button" @click="addProduct">Save</vs-button>
            </div>
          </vs-col>
        </vs-row>
       </vs-popup>
  ...
  methods: {
    ...
    addProduct (){
      if(this.productTitle == ''){
        ...
        this.$store.dispatch('productManage/addProduct', this.product).then(()=>{
             this.$vs.notify({
              color: this.notification_color,
              text: this.notification_text,
              icon: this.notification_icon
