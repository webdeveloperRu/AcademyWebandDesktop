    ...
    <vs-col vs-lg="8" vs-xs="12" vs-sm="12">
      <div class="mb-4">
        ...
        <div class="d-flex mt-3">
          <h2>Account User</h2>
          <vs-button style="margin-left: auto; height: 45px" @click="linkToNewUser">+ New User</vs-button>
        </div>
      </div>
    </vs-col>
    <!-- 
  ...
  <vs-table max-items="5" pagination :data="subuser_list">
    ...
    <template slot-scope="{data}">
      <vs-tr :key="indextr" v-for="(tr, indextr) in data">
        ...
        <vs-td :data="data[indextr].id"><i class="mdi mdi-pencil" style="cursor: pointer" @click="editSubUser(data[indextr].id)"></i></vs-td>
      </vs-tr>
    </template>
  </vs-table>
    </vs-card>
...
export default {
  ...
  methods: {
    ...
    **/
    getAllAccountUsers() {
      this.$store.dispatch('subuserManage/getSubUserList').then(()=>{
        this.$vs.notify({
