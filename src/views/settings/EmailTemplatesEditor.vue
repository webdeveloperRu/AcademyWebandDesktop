<template>
  <vs-row vs-justify="center">
    ...
    <vs-col vs-lg="8" vs-xs="12" vs-sm="12">
      <span class="ml-2 mb-2 mt-2 primary-font" @click="backToEmailTemplates" style="cursor: pointer;"><i class="ti-angle-left" style="font-size: 14px;"></i> Email Templates</span>
      <div class="mb-4 mt-3">
        <div class="d-flex">
        ...
        <div class="d-flex" style="justify-content: flex-start; align-items: center;">
          ...
          <span class="routing-product mr-3" @click="previewTemplate" style="cursor: pointer"><h5 class="primary-font"><i class="mdi mdi-eye" style="font-size: 14px;"></i> Preview Template</h5></span>

          <vs-dropdown vs-custom-content vs-trigger-click style="margin:0px; padding: 0px;">
              <span class="routing-product mr-3" style="cursor: pointer"><h5 class="primary-font"><i class="mdi mdi-email" style="font-size: 14px;"></i> Send Test Email</h5></span>
            <vs-dropdown-menu class="loginx">
              <div class="p-3">
                <vs-input
                  ...
                  v-model="test_email"
                />     
                <vs-button width="100%" color="primary" type="gradient" class="w-100"  @click="sendTestEmail">Send Test Email</vs-button>
              </div>
            </vs-dropdown-menu>
          </vs-dropdown>
        </div>
    ...
    >
      ...
      <vs-card class="email-template-code">
        ...
        <label class="mt-4">{{selected_template.help_guide}}
        </label>
      </vs-card>
        <div class="btn-alignment text-right">
          <vs-button class="mr-3" color="danger" type="flat"  @click="resetTemplate" v-if="visibleReset">Reset Template</vs-button>
          <vs-button color="primary" type="filled" @click="saveTemplate">Save</vs-button>
        </div>
    </vs-col>
  </vs-row>
</template>
  ...
  methods: {
    ...
    **/
    saveTemplate() {
      ...
      this.$store.dispatch('templateManage/updateTemplate', [this.selected_template, this.template_index]).then(()=>{
        this.$vs.notify({
    ...
    **/
    sendTestEmail() {
      this.$store.dispatch('templateManage/sendTestEmail', [this.selected_template, this.test_email]).then(()=>{
        this.$vs.notify({
          color: this.notification_color,
          text: this.notification_text,
          icon: this.notification_icon
    ...
    **/
    ...
    resetTemplate() {
      this.$store.dispatch('templateManage/resetTemplate', this.selected_template).then(()=>{
